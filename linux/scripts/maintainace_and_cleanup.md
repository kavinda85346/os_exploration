...
if command -v btrfs >/dev/null 2>&1 && mount | grep -q btrfs; then
  echo "btrfs detected — creating subvolume snapshot (edit SUBVOL and SNAPNAME)"
  # User MUST edit SUBVOL to the mounted subvolume path
  SUBVOL="/"    # <-- EDIT: path to btrfs subvolume to snapshot
  SNAPNAME="snap_${TIMESTAMP}"
  btrfs subvolume snapshot -r "$SUBVOL" "${SUBVOL}/.snapshots/${SNAPNAME}"
  echo "btrfs snapshot created at ${SUBVOL}/.snapshots/${SNAPNAME}"
elif command -v lvcreate >/dev/null 2>&1; then
  echo "LVM detected — creating snapshot (edit VG and LV names below)"
  VG="vg_name"    # <-- EDIT: volume group name
  LV="root_lv"    # <-- EDIT: logical volume name
  SNAPLV="${LV}_snap_${TIMESTAMP}"
  lvcreate --size 1G --snapshot --name "$SNAPLV" "/dev/${VG}/${LV}"
  echo "LVM snapshot created: /dev/${VG}/${SNAPLV}"
else
...
if [ -d "$HOME/.local/share/Trash" ]; then
  ...
  rm -rf "$HOME/.local/share/Trash/info"/* || true
  echo "Cleared XDG Trash"
fi
